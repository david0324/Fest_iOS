...
- (id)initWithFrame:(CGRect)frame {
    if ( (self = [super init]) ) {
        ...
        TEST_MODE;
        swipeLeft = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeLeftPlayer:)];
        swipeLeft.numberOfTouchesRequired = 1;
        swipeLeft.direction = UISwipeGestureRecognizerDirectionLeft;
        [self.view addGestureRecognizer:swipeLeft];
        
        swipeRight = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeRightPlayer:)];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(exitPlayerAndView:) name:@"ExitPlayer" object:nil];
        ...
        if (!_movieBackgroundView) {
            ...
            _movieBackgroundView.autoresizesSubviews = YES;
            _movieBackgroundView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin|UIViewAutoresizingFlexibleBottomMargin|UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
...
-(void)swipeLeftPlayer:(UISwipeGestureRecognizer *)gest
{
    DISPLAY_METHOD_NAME;
    TEST_MODE;
    [self setFullscreen:NO];
    [self.delegate exitPlayeSwipeLeft];
    TEST_MODE;
    
    //[self.delegate swipeLeftNotify];
}

-(void)swipeRightPlayer:(UISwipeGestureRecognizer *)gest
{
    ...
    TEST_MODE;
    [self setFullscreen:NO];
    [self.delegate exitPlayeSwipeRight];
    TEST_MODE;
    //[self.delegate swipeRightNotify];
}

#pragma mark - Notification
-(void)readyForDisplay:(NSNotification*)not{
    DISPLAY_METHOD_NAME;
}
-(void)exitPlayerAndView:(NSNotification *)notify
{
    [self.delegate exitPlayer];
}

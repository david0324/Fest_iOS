...
//    CGContextClearRect(context, rect);
    ...
    for (int i=0; i<portions.count; i++) {
        ...
        if (portionSum > 0 && portionSum < 1) {
            ...
            if(i==2)
            {
                draw1PxStroke(context, startPoint, endPoint, [UIColor blackColor].CGColor);
                
                float startX=startPoint.x+1;
                startPoint = CGPointMake(startX, 0);
                endPoint = CGPointMake(startX, rect.size.height);
                draw1PxStroke(context, startPoint, endPoint, [UIColor blackColor].CGColor);
            }
            else
            {
...
- (void)refreshTable:(UITableView *)table
{
    ...
    switch (table.tag) {
        ...
        case HOUR_TAG: {
            ...
            [self scrollToAndSelectIndex:(selectedHour-1) forTableView:table];
            break;
        }
        case MINUTE_TAG: {
        ...
        case AM_PM_TAG: {
            ...
            [self scrollToAndSelectIndex:selectedAM_PM forTableView:table];
            break;
        }
        default:
            break;
...
//                hasChanged = YES;
        ...
        case AM_PM_TAG: {
            NSInteger nowAM_PM = index;
            if (selectedAM_PM != nowAM_PM) {
                selectedAM_PM = nowAM_PM;
//                hasChanged = YES;
                [self callBackToUpdateDate];
            }
            break;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (tableView.tag) {
        case YEAR_TAG: {
            ...
            cell.label.text = [NSString stringWithFormat:@"%d",_startYear + indexPath.row];
            break;
        }
        case MONTH_TAG: {
            
            cell.label.text =[self MonthNameString:indexPath.row];// [NSString stringWithFormat:@"%d",indexPath.row + 1];
            break;
        }
        case DAY_TAG: {
        ...
        case MINUTE_TAG: {
            ...
            cell.label.text = [NSString stringWithFormat:@"%02d",indexPath.row];
            break;
        }
        case AM_PM_TAG: {

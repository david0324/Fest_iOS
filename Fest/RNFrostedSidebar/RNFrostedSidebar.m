...
const NSUInteger tagProfileView = 100;
...
- (void)layoutSubviews {
    ...
    TEST_MODE;
    if (self.tag==tagProfileView) {
//        no need to set here it will be set while setting the image****
        
//        [self.imageView setFrame:CGRectMake(0, 0, inset*2, inset*2)];
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            if (idx==0) {
//                profile **
                view.tag=tagProfileView;
                

                
                CGFloat inset = self.itemSize.width/2;
                [view.imageView setFrame:CGRectMake(0, 0, inset, inset)];
                view.imageView.center = CGPointMake(inset, inset);
                
                UserModel *user = [[GC arrUserDetails] firstObject];
                NSString *path=[ILLogic savingPathForProfileImage:user.facebookID];
                if ([FILE_MGR fileExistsAtPath:path]) {
                    [self setProfileImageToView:view];
                }

                NSURL *urlProfImage=[NSURL URLWithString:user.profileURL];
                NSURLSession *session      = [NSURLSession sharedSession];
                
                NSMutableURLRequest *theRequest=[NSMutableURLRequest requestWithURL:urlProfImage];
                [[session dataTaskWithRequest:theRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                    if(!error)
                    {
                        NSError *error;
                        
                        //            NSLog(@"news response --%@",responseDict);
...
-(void)setProfileImageToView:(RNCalloutItemView*)view{
    UserModel *user = [[GC arrUserDetails] firstObject];
    NSString *path=[ILLogic savingPathForProfileImage:user.facebookID];

    dispatch_async(dispatch_get_main_queue(), ^{
        UIImage *imageNew=[UIImage imageWithContentsOfFile:path];
        view.imageView.image = imageNew;
        [view.imageView setFrame:CGRectMake(0, 0, self.itemSize.width, self.itemSize.width)];

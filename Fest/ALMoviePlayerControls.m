...
static const CGFloat iPhoneScreenPortraitWidth = 320;
...
- (void)setup {
    ...
    TASK_LISTED;
    /*
//    Redo UI of video Just play button and seekbar
     */
...
- (void)fullscreenPressed:(UIButton *)button {
    ...
    [self performSelector:@selector(exitMoviePlayer:) withObject:nil afterDelay:0.1];
}

-(void)exitMoviePlayer:(id)sender
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ExitPlayer" object:nil];
}
...
- (void)layoutSubviews {
    ...
    /*
     Redo UI of video Just play button and seekbar 
     */
    if (_fullscreenButton.hidden) {
        self.timeRemainingLabel.frame = CGRectMake(_fullscreenButton.frame.origin.x, 0, labelWidth, self.barHeight);
    }
    TASK_LISTED;
